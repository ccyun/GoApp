...
func (db *DB) alterTable() error {
	sqls := []string{
		...
		//更新选项表
		"CREATE TABLE `bbs_bbs_task_option` (`id`  bigint(20) UNSIGNED NOT NULL AUTO_INCREMENT ,`site_id`  bigint(20) UNSIGNED NOT NULL ,`board_id`  bigint(20) UNSIGNED NOT NULL ,`bbs_id`  bigint(20) UNSIGNED NOT NULL ,`sub_task_id`  bigint(20) UNSIGNED NOT NULL COMMENT '子任务ID' ,`type`  tinyint(4) UNSIGNED NOT NULL COMMENT '1,单选，2多选' ,`group_name`  varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL COMMENT '矩阵选择组名称' ,`name`  varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL COMMENT '选项名称' ,`image`  varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL COMMENT '选项图片' ,`value`  int(10) NOT NULL COMMENT '选项值，数字' ,`is_ext_value`  tinyint(1) UNSIGNED NOT NULL COMMENT '是否有其他值' ,`selected`  tinyint(1) UNSIGNED NOT NULL COMMENT '是否默认选中' ,`show_sub_task_id`  bigint(20) UNSIGNED NOT NULL COMMENT '选中后显示哪个子任务' ,PRIMARY KEY (`id`),INDEX `site_id` (`site_id`) USING BTREE ,INDEX `board_id` (`board_id`) USING BTREE ,INDEX `bbs_id` (`bbs_id`) USING BTREE ,INDEX `sub_task_id` (`sub_task_id`) USING BTREE ,INDEX `value` (`value`) USING BTREE ) ENGINE=InnoDB DEFAULT CHARACTER SET=utf8mb4 COLLATE=utf8mb4_general_ci ROW_FORMAT=Compact",
	...
	for i := 0; i < tableNum; i++ {
		...
		if _, err := d.Raw("CREATE TABLE `" + fmt.Sprintf("bbs_msg_%.4d", i) + "` (`id`  bigint(20) UNSIGNED NOT NULL AUTO_INCREMENT ,`site_id`  bigint(20) UNSIGNED NOT NULL ,`board_id`  bigint(20) UNSIGNED NOT NULL ,`discuss_id`  bigint(20) UNSIGNED NOT NULL ,`bbs_id`  bigint(20) UNSIGNED NOT NULL ,`feed_type`  enum('bbs','form','task','taskReply','taskAudit','taskClose') CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL COMMENT 'bbs广播,form表单,task任务,taskReply任务反馈提醒,taskAudit审核提醒,taskClose任务关闭' ,`feed_id`  bigint(20) UNSIGNED NOT NULL ,`user_id`  bigint(20) UNSIGNED NOT NULL ,`user_org_id`  bigint(20) UNSIGNED NOT NULL ,`task_status` tinyint(1) unsigned NOT NULL,`is_read`  tinyint(1) UNSIGNED NOT NULL COMMENT '是否已读' ,`created_at`  bigint(13) UNSIGNED NOT NULL ,PRIMARY KEY (`id`),INDEX `board_id` (`board_id`) USING BTREE ,INDEX `feed_type` (`feed_type`) USING BTREE ,INDEX `user_id` (`user_id`) USING BTREE ,INDEX `bbs_id` (`bbs_id`) USING BTREE ,INDEX `is_read` (`is_read`) USING BTREE ,INDEX `discuss_id` (`discuss_id`) USING BTREE ,INDEX `feed_id` (`feed_id`) USING BTREE ) ENGINE=InnoDB DEFAULT CHARACTER SET=utf8mb4 COLLATE=utf8mb4_general_ci ROW_FORMAT=Compact").Exec(); err != nil {
			return err
		}
	}
	return nil
